...
class App:
    ...
    def check_source(self, source: Source) -> AvailableSource | None:
        for url in source.urls:
            ...
            if source.encrypted:
                logger.debug(f"⏳ 解密数据源: {source.name} - {base_url}")
                raw = self.http_client.get(base_url)
                if raw:
                    try:
                        decrypted = decode(raw)
                        if self._check_content_valid(decrypted):
    ...
    def run(self) -> None:
        ...
        available_sources = [result for source in sources if (result := self.check_source(source))]
        ...
        build_and_save(available_sources)
